...
def main():
    ...
    print(f"\n  Fetching Place Details for {len(top10)} leads...")
    top10, details_live, details_cost = enrich_with_details(top10)
    if details_live:
        print(f"  Place Details: {details_live} live (${details_cost:.2f}), "
              f"{len(top10) - details_live} cached ($0.00)")
    else:
        print(f"  Place Details: all {len(top10)} served from cache ($0.00)")
